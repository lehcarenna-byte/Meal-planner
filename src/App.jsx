...
// ── System prompt ──────────────────────────────────────────────────────────
const SYSTEM = `You are a budget meal planning assistant for a family of 3 in California.
...
- Tags must be one of: "Batch Cook", "15 min cook", "One Pan", "Oven Meal", "Budget MVP", "High Protein", "No Waste"
...
// ── Meal card ──────────────────────────────────────────────────────────────
function MealCard({ meal, index, expanded, onToggle, onRegenerate, regenLoading }) {
  ...
  return (
    <div style={{
      background: "#fff",
      border: `1.5px solid ${isOpen ? tc : "#e8e0d8"}`,
      borderRadius: 16,
      marginBottom: 10,
      overflow: "hidden",
      boxShadow: isOpen
        ? `0 6px 24px rgba(0,0,0,0.09), inset 3px 0 0 ${tc}`
      ...
      <div
        onClick={onToggle}
        style={{ display: "flex", alignItems: "center", padding: "16px 18px", gap: 12, cursor: "pointer" }}
      >
        <div style={{
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontFamily: "'Instrument Serif', Georgia, serif", fontSize: 16, fontWeight: 600, color: "#1c1917", marginBottom: 2, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
            {meal.name}
          </div>
          <div style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 11, color: "#9a8f85", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
            {meal.description}
          </div>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 5, flexShrink: 0 }}>
          ...
          <div style={{ fontFamily: "'Instrument Serif', Georgia, serif", fontSize: 17, fontWeight: 700, color: "#c97d30" }}>
            ${meal.cost?.toFixed(2)}
          </div>
        </div>
      ...
      {isOpen && (
        <div style={{ borderTop: `1px solid ${tc}28`, background: "#faf8f5" }}>
          ...
          <div style={{ display: "flex", borderBottom: "1px solid #ede9e3" }}>
            <div style={{ flex: 1, padding: "10px 18px", fontFamily: "'DM Sans', sans-serif", fontSize: 12, color: "#7a6f67", borderRight: "1px solid #ede9e3" }}>
              ⏱ Prep <strong style={{ color: "#1c1917" }}>{meal.prepTime}</strong>
            </div>
            <div style={{ flex: 1, padding: "10px 18px", fontFamily: "'DM Sans', sans-serif", fontSize: 12, color: "#7a6f67" }}>
              🔥 Cook <strong style={{ color: "#1c1917" }}>{meal.cookTime}</strong>
            </div>
          </div>
            ...
            <div style={{ padding: "16px 18px" }}>
              ...
              {(meal.instructions || []).map((step, j) => (
                <div key={j} style={{ display: "flex", gap: 8, fontFamily: "'DM Sans', sans-serif", fontSize: 12, color: "#4a403a", padding: "3px 0", lineHeight: 1.5 }}>
                  <span style={{ fontFamily: "'Instrument Serif', serif", fontWeight: 700, color: tc, flexShrink: 0, minWidth: 14 }}>{j+1}.</span>
                  {step}
                </div>
              ))}
            </div>
          ...
          {/* Regenerate button */}
          <div style={{ padding: "12px 18px", borderTop: "1px solid #ede9e3", display: "flex", justifyContent: "flex-end" }}>
            <button
              onClick={(e) => { e.stopPropagation(); onRegenerate(index); }}
              disabled={regenLoading === index}
              style={{
                display: "flex",
                alignItems: "center",
                gap: 6,
                padding: "7px 14px",
                background: regenLoading === index ? "#ede9e3" : "#1c1917",
                color: regenLoading === index ? "#9a8f85" : "#fff",
                border: "none",
                borderRadius: 8,
                fontFamily: "'DM Sans', sans-serif",
                fontSize: 12,
                fontWeight: 600,
                cursor: regenLoading === index ? "not-allowed" : "pointer",
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    padding: "10px 14px",
                    gap: 10,
                    borderBottom: ii < cat.items.length - 1 ? "1px solid #f2ede8" : "none",
                    background: ii % 2 === 0 ? "#fff" : "#fdfaf7",
                    cursor: "pointer",
                    opacity: done ? 0.45 : 1,
                ...
                >
                  <div style={{
                    ...
                    border: `1.5px solid ${done ? cat.color || "#e8a838" : "#c8bfb8"}`,
                    background: done ? (cat.color || "#e8a838") : "transparent",
                    display: "flex", alignItems: "center", justifyContent: "center",
                    transition: "all 0.15s",
                  }}>
                    {done && <span style={{ color: "#fff", fontSize: 10, lineHeight: 1 }}>✓</span>}
                  </div>
                  <div style={{ flex: 1, fontFamily: "'DM Sans', sans-serif", fontSize: 13, color: "#1c1917", textDecoration: done ? "line-through" : "none" }}>
                    {item.name}
                  </div>
...
export default function MealPlannerApp() {
  ...
  const regenMeal = useCallback(async (index) => {
    ...
    const prompt = [
      ...
      `Other meals this week are: ${otherMeals}. Try to reuse their ingredients where possible.`,
    ...
    try {
      ...
      // silently fail — keep existing meal
    }
    setRegenLoading(null);
  }, [meals, anchors, weekTotal]);
    ...
    <div style={{
      ...
      <div style={{ background: "#1c1917", padding: "52px 24px 44px", textAlign: "center" }}>
        <div style={{ maxWidth: 420, margin: "0 auto" }}>
          <div className="fade-up" style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 11, color: "#c97d30", letterSpacing: "0.22em", textTransform: "uppercase", marginBottom: 16 }}>
            AI Meal Planner · California
          </div>
          ...
          <p className="fade-up" style={{ fontSize: 14, color: "#7a6f67", lineHeight: 1.7, animationDelay: "0.2s" }}>
      ...
      <div style={{ maxWidth: 440, margin: "0 auto", padding: "32px 20px 60px" }}>
        ...
        {error && (
          <div style={{ background: "#fef2f2", border: "1px solid #fca5a5", borderRadius: 10, padding: "12px 16px", marginBottom: 20, fontFamily: "'DM Sans', sans-serif", fontSize: 13, color: "#b91c1c" }}>
            {error}
          </div>
        )}

        {/* Anchor ingredients */}
        <div style={{ marginBottom: 18 }} className="fade-up" style={{ animationDelay: "0.3s" }}>
          ...
          <input
            ...
            placeholder="e.g. chicken thighs, carrots, garlic…"
            style={{
              width: "100%",
              padding: "13px 16px",
              background: "#fff",
        ...
        <div style={{ marginBottom: 18 }}>
          ...
          <textarea
            className="field-input"
            value={anchorRecipe}
            onChange={e => setAnchorRecipe(e.target.value)}
            placeholder="e.g. chicken stir-fry with broccoli, or my grandmother's lentil soup…"
            rows={2}
            style={{
              width: "100%",
              padding: "13px 16px",
        ...
        <div style={{ marginTop: 24 }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            {[
              { label: "🥩 Beef & potatoes week", anchors: "ground beef, potatoes, onions", recipe: "" },
              { label: "🍗 Chicken & rice focus", anchors: "chicken thighs, rice, garlic", recipe: "" },
              { label: "🌱 Mostly meatless", anchors: "lentils, chickpeas, seasonal veg", recipe: "" },
              { label: "🍝 Pasta night anchor", anchors: "pasta, ground beef, carrots", recipe: "spaghetti bolognese" },
            ].map(ex => (
              <button
                key={ex.label}
                onClick={() => { setAnchors(ex.anchors); setAnchorRecipe(ex.recipe); }}
                style={{
                  padding: "7px 12px",
                  background: "#fff",
                  border: "1.5px solid #e8e0d8",
  ...
  if (screen === "loading") return (
    <div style={{
      ...
      <div style={{ width: "100%", maxWidth: 440, marginTop: 36 }}>
        {DAYS.map(d => <SkeletonCard key={d} day={d} />)}
      </div>
    </div>
  );
      ...
      <div style={{ background: "#1c1917", padding: "20px 20px 0" }}>
        <div style={{ maxWidth: 500, margin: "0 auto" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18 }}>
            ...
            <div style={{ textAlign: "right" }}>
              <div style={{ fontFamily: "'Instrument Serif', Georgia, serif", fontSize: 26, color: "#e8a560" }}>~${weekTotal?.toFixed(0) || totalCost.toFixed(0)}</div>
              <div style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 10, color: "#7a6f67" }}>est. total shop</div>
            </div>
          </div>
            ...
            {[["meals","🍽 Meals"],["groceries","🛒 Grocery List"]].map(([id, label]) => (
              <button
                ...
                style={{
                  ...
                  color: activeTab === id ? "#e8a560" : "#7a6f67",
                  borderBottom: activeTab === id ? "2px solid #c97d30" : "2px solid transparent",
                  marginBottom: -1,
                }}
              >{label}</button>
            ))}
      ...
      <div style={{ maxWidth: 500, margin: "0 auto", padding: "20px 16px 80px" }}>
        {activeTab === "meals" && (
          <div className="fade-in">
            {meals.map((meal, i) => (
              <MealCard
                key={`${meal.day}-${meal.name}`}
                meal={meal}
                index={i}
                expanded={expanded}
                onToggle={() => setExpanded(expanded === i ? null : i)}
                onRegenerate={regenMeal}
